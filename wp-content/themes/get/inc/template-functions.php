 ...
 * @return array
 */
function get_theme_body_classes( $classes ) {
...
add_filter( 'body_class', 'get_theme_body_classes' );
 ...
 * @return array
 */
function get_theme_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'get_theme_post_classes', 10, 3 );
 ...
 * @return void
 */
function get_theme_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'get_theme_pingback_header' );
 ...
 * @return void
 */
function get_theme_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'get_theme_supports_js' );
 ...
 * @return array
 */
function get_theme_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'get_theme_comment_form_defaults' );
 ...
 * @return bool
 */
function get_theme_can_show_post_thumbnail() {
 ...
 * @return int
 */
function get_theme_get_avatar_size() {
	return 60;
}
 ...
 */
function get_theme_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'gettheme' ),
 ...
 */
function get_theme_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . get_theme_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'get_theme_continue_reading_link_excerpt' );

/**
 * Create the continue reading link.
 */
function get_theme_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . get_theme_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'get_theme_continue_reading_link' );

if ( ! function_exists( 'get_theme_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function get_theme_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'gettheme' ) : $title;
	}
}
add_filter( 'the_title', 'get_theme_post_title' );
...
function get_theme_get_icon_svg( $group, $icon, $size = 24, $fill="#000" ) {
	return get_theme_SVG_Icons::get_svg( $group, $icon, $size, $fill );
}
 ...
 * @return string
 */
function get_theme_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? get_theme_get_icon_svg( 'ui', 'arrow_right' ) : get_theme_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? get_theme_get_icon_svg( 'ui', 'arrow_left' ) : get_theme_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'get_theme_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function get_theme_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'get_theme_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'get_theme_get_localized_font_family_elements',
		array(
	...
	return get_theme_generate_css( // @phpstan-ignore-line.
 ...
 */
function get_theme_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function get_theme_password_form( $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'gettheme' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label ) . '">' . esc_html_x( 'Password', 'Post password form', 'gettheme' ) . '</label><input class="post-password-form__input" name="post_password" id="' . esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="' . esc_attr_x( 'Submit', 'Post password form', 'gettheme' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'gettheme' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'get_theme_password_form' );
 ...
 * @return array
 */
function get_theme_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'get_theme_get_attachment_image_attributes', 10, 3 );